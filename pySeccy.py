...
PSO_LEGACY_CLASSVAL = 5 # Calculations for PSO before BB assume class is a Racaseal
...
if ((modechar == 'c') or (modechar == "C")): # Classic calculation mode
    ...
    while (1):
        ...
        if (isascii(charname)): # Check if all characters in string are ASCII
            if (len(charname) <= 12):
                print(PSO_SECTIONID[(unicode_sum(charname) + PSO_LEGACY_CLASSVAL) % 10]) # Get the right section ID in the table and print it
                break
            else:
                print("\n" + LEN_REMINDER + "\n")
        else:
